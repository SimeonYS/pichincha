...
class PpichinchaPipeline:

    # Database setup
    conn = sqlite3.connect('pichincha.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `pichincha`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM pichincha WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO pichincha (date, title, link, content)"
